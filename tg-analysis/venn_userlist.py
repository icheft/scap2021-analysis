...
# [x[1]['title'] for x in sorted(list(j.items()), key=lambda a: len(a[1]['title']))]
    ...
    sorted_j = [x for x in sorted(list(j.items()), key=lambda a: len(
        a[1]['title'])) if(len(x[1]['users']) > 0)]
    ...
    for index, name in enumerate(names):
        ...
        for chat in sorted_j:
            ...
            if name.lower() in chat[1]['title'].lower() and len(chat[1]['users']) > 0 and (not found_chat):
                ...
                userlists[index].extend([user['id']
                                        for user in chat[1]['users']])
                found_chat = True
    ...
    if len(userlists) == 2:
        venn2([set(userlists[0]), set(userlists[1])],
              (full_names[0], full_names[1]))
    elif len(userlists) == 3:
        venn3([set(userlists[0]), set(userlists[1]), set(userlists[2])],
              (full_names[0], full_names[1], full_names[2]))
    ...
    if savefolder is not None:
        ...
        if len(names_string) > 200:
            ...
            figname = 'User overlap in {}'.format(
                names_string).replace('/', '_')
