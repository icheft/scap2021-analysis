...
def make_ddict_in_range(json_file, binsize, start, end):
    ...
    dates_and_lengths = (extract_date_and_len(event)
                         for event in events if 'text' in event)
    dates_and_lengths = ((date, length) for (date, length)
                         in dates_and_lengths if date >= start and date <= end)
...
def main():
    ...
    for ind, filepath in enumerate(filepaths):
        ...
        if "fug" in filepath:
            plt.plot(*zip(*chat_activity), color="#f4af00")
        elif "shi" in filepath:
            plt.plot(*zip(*chat_activity), color="#D10F24")
        plt.grid()
