...
def extract_infos(event):
    ...
    try:
        ...
        text_userid = event['actor_id']
        text_printname = event['actor']
...
def make_ddict(jsonfile, start, end):
    ...
    messages = ((when, what, uid, who) for (when, what, uid, who)
                in messages if when >= start and when <= end)
...
def main():
    ...
    with open(filepath, 'r') as jsonfile:
        names, total_datapoints, counter = make_ddict(
            jsonfile, start_date, end_date)

    trimmedCounter = make_trimmed_ddict(
        counter, total_datapoints, names, other_percent)
    ...
    color = color_palette(len(freqList[0]), 'tab20')  # gnuplot
    plt.pie(freqList[1], labels=freqList[0],
            startangle=135, textprops={'fontproperties': font}, colors=color)
...
#    plt.set_lw(10)
    ...
    if savefolder is not None:
        ...
        plt.savefig(
            "{}/Most active users in {}.png".format(savefolder, filename))
    else:
