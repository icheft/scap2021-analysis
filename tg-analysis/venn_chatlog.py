...
def main():
    ...
    for index, filepath in enumerate(filepaths):
        ...
        with open(filepath, 'r') as jsonfile:
            ...
            for event in events:
                if "action" in event and event["action"] == "invite_members":
                    #print(event['action']['user']['id'], ":", event['action']['user']['print_name'])
                    users[index].add(event['actor_id'])
                elif "action" in event and event['action'] == "join_group_by_link":
